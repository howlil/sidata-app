...
import editDaftarTA from "@/apis/mhs/TA/editDaftarTA";
...
import getDetailDaftarTAByMhsiswa from "@/apis/mhs/TA/getDetailDaftarTAByMhsiswa";

export default function EditDaftarTA() {
  const id = getDataFromToken()?.userId;
  const [data, setData] = useState(null);
  const [status, setStatus] = useState({ status: "", statusTA: "" });
  const [daftarTAId, setDaftarTAId] = useState(null); 
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await getDetailDaftarTAByMhsiswa(id);
        if(res.success){
          setDaftarTAId(res.data.daftarTAId);
        }
        if (response.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await editDaftarTA(
        daftarTAId,
        id,
        data.idTA,
  ...
  return (
    <Layout>
      ...
      <div className="space-y-3 mb-4">
        <h1 className="font-bold text-2xl mb-8">Edit Pendaftaran TA</h1>
