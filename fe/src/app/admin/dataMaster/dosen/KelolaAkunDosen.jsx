...
export default function KelolaAkunDosen() {
  ...
  useEffect(() => {
    if (id) {
      ...
      const fetchAkunDosen = async () => {
        try {
          ...
          if (response.success) {
            const { nama, nip, email, jabatanId, bidangDosen } = response.data;
            setNama(nama);
            setNip(nip);
            setEmail(email);
    ...
    const fetchJabatanOptions = async () => {
      try {
        ...
        if (response.success) {
          setJabatanOptions(response.data.map(jabatan => ({
            value: jabatan.id,
            label: jabatan.nama
          })));
        } else {
    ...
    const fetchBidangOptions = async () => {
      try {
        ...
        if (response.success) {
          setBidangOptions(response.data.map(bidang => ({
            value: bidang.id,
            name: bidang.nama,
            details: bidang.details
          })));
        } else {
          setError(response.message);
        }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      let response;
      if (isEditing) {
        response = await editAkunDosen(nama, nip, email, password, jabatanId, bidangDosen, id);
      } else {
        response = await addAkunDosen(nama, nip, email, password, jabatanId, bidangDosen);
      }

      if (response.success) {
        setSuccess('Data dosen berhasil disimpan!');
        navigate("/manajemenDosen"); 
      } else {
        setError(response.message);
      }
    } catch (error) {
  ...
  return (
    <Layout>
      <h1 className="font-bold text-2xl mb-8">{isEditing ? "Edit" : "Tambah"} Data Dosen</h1>
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        {!isEditing && <Input label="Password" value={password} onChange={(e) => setPassword(e.target.value)} type="password" />}
        <Select label="Jabatan" options={jabatanOptions} value={jabatanId} onChange={(e) => setJabatanId(e.target.value)} />
        <InputCheck label="Bidang" items={bidangOptions} onSelect={handleSelectBidang} />
      ...
      {error && <Toast message={error} isVisible={!!error} onClose={() => setError(null)} isSuccess={false} />}
      {success && <Toast message={success} isVisible={!!success} onClose={() => setSuccess(null)} isSuccess={true} />}
    </Layout>
  );
}
