...
export default function KelolaAkunMhs() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.success) {
        setSuccess(isEditing ? 'Data mahasiswa berhasil diperbarui!' : 'Data mahasiswa berhasil ditambahkan!');
        if (!isEditing) {
          setNama('');
          setEmail('');
          setPassword('');
  ...
  return (
    <Layout>
      <h1 className="font-bold text-2xl mb-8">{isEditing ? 'Edit Data Mahasiswa' : 'Tambah Data Mahasiswa'}</h1>
      <form className="space-y-4" onSubmit={handleSubmit}>
        <Input label="Nama Lengkap" value={nama} onChange={(e) => setNama(e.target.value)} />
        <Input label="NIM" value={nim} onChange={(e) => setNim(e.target.value)} />
        ...
        <div className="flex justify-end">
          <Button custom="mt-8" type="submit" disabled={loading}>
            {loading ? (isEditing ? 'Memperbarui...' : 'Menambahkan...') : (isEditing ? 'Perbarui Data' : 'Tambah Data')}
          </Button>
        </div>
      </form>
