...
const { status, statusTA, tipePembimbing } = require("../../config/typeEnum");
...
exports.accIdeTA = async (req, res) => {
    try {
        ...
        const advisor = await prisma.dosen.findUnique({ where: { idDosen: dosenId } });
        if (!advisor) {
        ...
        const totalAdvisors = await prisma.dosenPembimbingTA.count({ where: { idTA } });
...
exports.accJudulTA = async (req, res) => {
    try {
      ...
      res.status(200).json({
        success: true,
        message: `Judul TA ${isApproved ? "disetujui" : "ditolak"} oleh pembimbing`,
      });
    } catch (error) {
