...
exports.getRiwayatBimbingan = async (req, res) => {
...
exports.editJadwalBimbingan = async (req, res) => {
    try {
        await editJadwalBimbinganSchema.validate(req.body);

        const { idJadwal, kendala, progresTA, tanggal, waktuMulai, waktuSelesai } = req.body;

        const existingJadwal = await prisma.jadwalBimbinganDosen.findUnique({ where: { id: idJadwal } });
        if (!existingJadwal) {
            return res.status(404).json({ success: false, message: "Jadwal bimbingan tidak ditemukan" });
        }

        if (existingJadwal.status !== status.ditolak) {
            return res.status(400).json({ success: false, message: "Hanya jadwal bimbingan yang ditolak yang dapat diedit" });
        }
